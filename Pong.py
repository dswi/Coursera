...
def draw(canvas):
    ...
    # draw scores
    canvas.draw_text(str(score1), [WIDTH / 4, 40], 36, "White", "sans-serif")
    canvas.draw_text(str(score2), [WIDTH - WIDTH / 4, 40], 36, "White", "sans-serif")
    
def keydown(key):
    global paddle1_vel, paddle2_vel
    if key == simplegui.KEY_MAP["s"]:
        paddle1_vel += pad_motion
    elif key == simplegui.KEY_MAP["w"]:
        paddle1_vel -= pad_motion
    elif key == simplegui.KEY_MAP["down"]:
        paddle2_vel += pad_motion
