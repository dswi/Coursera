...
def draw(canvas):
    global time, started, rock_group, lives, score, missile_group, my_ship
...
def rock_spawner():
    global rock_group, a_rock, started
...
def process_sprite_group(some_set, canvas):
    ...
    for i in set(some_set):
        if i.update():
            some_set.remove(i)

def group_collide(some_set, other_object):
    for i in set(some_set):
